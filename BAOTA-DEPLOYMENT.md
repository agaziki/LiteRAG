...
**易错点**：启动选项选成 `dev` 会以开发模式启动——Vite 会在 5173 端口起一个前端开发服务器并对所有网卡监听，且后端带文件监视。生产环境启动选项必须选 `start`（单端口 3000）。关于 5173 的完整说明见下方常见问题表。
...
| 进程里出现 5173 端口 / 想用 5173 访问 | 5173 是 **Vite 开发服务器**的默认端口，仅在本地执行 `npm run dev` 时存在，用于改代码热更新。生产部署用 `npm start`，只有 3000 一个端口，**无需也勿开放 5173**；若服务器上出现 5173 说明误跑了 dev 模式，停掉换 `npm start` |
