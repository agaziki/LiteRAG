...
### 方式 A：面板「Node项目」模块（新版宝塔推荐）

宝塔「网站」→ 顶部切换到 **Node项目** → 添加Node项目：

| 配置项 | 值 |
|--------|-----|
| 项目目录 | `/www/wwwroot/LiteRAG` |
| 启动选项 | `npm start`（或启动文件方式：运行用户 www，启动文件 `server/index.ts`） |
| Node 版本 | 22.x |
| 项目端口 | `3000` |
| 映射域名 | 你的域名（面板会自动建站并生成反代，仍需按第 6 步调整） |

提交后面板会以守护方式拉起项目，日志在项目详情中查看。
