...
- 👨‍💼 **自动转人工** - 高风险/敏感/无法解决场景自动触发，后台可「接入 / 标记解决」
...
> 模型列表在 `server/deepseek-agent.ts` 的 `getDeepSeekModels()` 中维护，默认模型为 `deepseek-v4-flash`。

## 图片问答（视觉模型）

1. 在输入框左下角点击 🖼 图标选择图片（或多次选择，最多 4 张，单张 ≤5MB，支持 png/jpeg/webp/gif）
2. 模型下拉切换到 `deepseek-v4-flash-vision-exp`（附带图片但模型不支持时会有黄色提示）
3. 发送后图片以缩略图显示在消息气泡上方，点击可放大查看
...
├── scripts/                       # 测试脚本（检索/导入/RAG 三套，npm test 一键运行）
...
完整部署流程（PM2 进程守护、nginx 反向代理 SSE 配置、数据备份、升级与故障排查）见 [DEPLOYMENT.md](./DEPLOYMENT.md)；二次开发指南见 [DEVELOPMENT.md](./DEVELOPMENT.md)。
